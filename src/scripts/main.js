...
salesByWeek.forEach(sale => {
   let keys =  Object.keys(sale.vehicle)
   console.log(keys)
   
        container.innerHTML += `
        <h1>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}</h1>
        <p>${keys[0]}: ${sale.vehicle.year}</p>
        <p>${keys[1]}: ${sale.vehicle.model}</p>
        <p>${keys[2]}: ${sale.vehicle.make}</p>
        <p>${keys[3]}: ${sale.vehicle.color}</p>
        <h2>$${sale.gross_profit}</h2>
        `
